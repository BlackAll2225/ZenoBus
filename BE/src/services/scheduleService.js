    ...
    try {
      ...
      for (const conflict of busConflicts.recordset) {
        ...
        if (conflictArrival && this.isTimeOverlap(newDepartureTime, newArrivalTime, conflictDeparture, conflictArrival)) {
          const vnDeparture = new Date(conflictDeparture.getTime() + 14 * 60 * 60 * 1000);
          const vnArrival = new Date(conflictArrival.getTime() + 14 * 60 * 60 * 1000);
          throw new Error(`Xe bus đã có lịch trình từ ${vnDeparture.toLocaleString('vi-VN', { hour12: false })} đến ${vnArrival.toLocaleString('vi-VN', { hour12: false })}`);
        }
      }
      ...
      for (const conflict of driverConflicts.recordset) {
        ...
        if (conflictArrival && this.isTimeOverlap(newDepartureTime, newArrivalTime, conflictDeparture, conflictArrival)) {
          const vnDeparture = new Date(conflictDeparture.getTime() + 14 * 60 * 60 * 1000);
          const vnArrival = new Date(conflictArrival.getTime() + 14 * 60 * 60 * 1000);
          throw new Error(`Tài xế đã có lịch trình từ ${vnDeparture.toLocaleString('vi-VN', { hour12: false })} đến ${vnArrival.toLocaleString('vi-VN', { hour12: false })}`);
        }
      }
