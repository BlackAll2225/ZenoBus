...
-- ========================================
INSERT INTO routes (departure_province_id, arrival_province_id, distance_km, estimated_time) VALUES 
-- Hà Nội - Hồ Chí Minh
(1, 2, 1720, 1800),
(2, 1, 1720, 1800),
        ...
        SELECT @Floor = 
            CASE bt.name
                WHEN N'Giường nằm' THEN 
                    CASE 
                        WHEN @i <= @SeatCount/2 THEN N'Tầng 1'
                        ELSE N'Tầng 2'
                    END
                WHEN N'Limousine' THEN N'Tầng 1'
                WHEN N'Ghế VIP' THEN N'Tầng 1'
                ELSE N'Tầng 1'
            END
        FROM schedules s
        JOIN buses b ON s.bus_id = b.id
...
PRINT 'Feedbacks inserted successfully!';

-- ========================================
-- THÔNG BÁO HOÀN THÀNH
-- ========================================
PRINT 'Dummy data created successfully with Schedule Automation support!';
PRINT 'Total records created:';
PRINT '- Provinces: ' + CAST((SELECT COUNT(*) FROM provinces) AS NVARCHAR(10));
PRINT '- Stops: ' + CAST((SELECT COUNT(*) FROM stops) AS NVARCHAR(10));
PRINT '- Bus Types: ' + CAST((SELECT COUNT(*) FROM bus_types) AS NVARCHAR(10));
PRINT '- Buses: ' + CAST((SELECT COUNT(*) FROM buses) AS NVARCHAR(10));
PRINT '- Drivers: ' + CAST((SELECT COUNT(*) FROM drivers) AS NVARCHAR(10));
PRINT '- Routes: ' + CAST((SELECT COUNT(*) FROM routes) AS NVARCHAR(10));
PRINT '- Staff Accounts: ' + CAST((SELECT COUNT(*) FROM staff_accounts) AS NVARCHAR(10));
PRINT '- Schedule Patterns: ' + CAST((SELECT COUNT(*) FROM schedule_patterns) AS NVARCHAR(10));
PRINT '- Schedules: ' + CAST((SELECT COUNT(*) FROM schedules) AS NVARCHAR(10));
PRINT '- Schedule Management: ' + CAST((SELECT COUNT(*) FROM schedule_management) AS NVARCHAR(10));
PRINT '- Seats: ' + CAST((SELECT COUNT(*) FROM seats) AS NVARCHAR(10));
PRINT '- Users: ' + CAST((SELECT COUNT(*) FROM users) AS NVARCHAR(10));
PRINT '- Bookings: ' + CAST((SELECT COUNT(*) FROM bookings) AS NVARCHAR(10));
PRINT '- Booking Seats: ' + CAST((SELECT COUNT(*) FROM booking_seats) AS NVARCHAR(10));
PRINT '- Payments: ' + CAST((SELECT COUNT(*) FROM payments) AS NVARCHAR(10));
PRINT '- Feedbacks: ' + CAST((SELECT COUNT(*) FROM feedbacks) AS NVARCHAR(10));